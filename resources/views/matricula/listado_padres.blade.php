        ...
        <table class="table table-striped" id="matriculados">
            ...
            <tr>
                ...
                <td>@foreach($padre->estudiantes as $hijos)
                    {{$hijos->user->nombre}} {{$hijos->user->apellido}}
                @endforeach
                </td>

                <td>{!!link_to_route('Detalle.Padre', $title = 'Detalles', $parameters = $padre->id, $attributes = ['class'=>'btn btn-success','onclick'=>"waitingDialog.show('Cargando... ',{ progressType: 'info'});setTimeout(function () {waitingDialog.hide();}, 3000);"])!!}</td>
            </tr>
                @endforeach
            </tbody>
        </table>
